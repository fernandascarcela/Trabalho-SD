...
def criar_atendimento(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_agendamento:5002/{endpoint}/atendimentos/criar",
            json=payload
        )
...
def excluir_atendimento(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_agendamento:5002/{endpoint}/atendimentos/excluir",
            json=payload
        )
        print(f"\n>>> Resposta do Servidor ({args.perfil_operador.upper()}):")
        print(resp.json())
...
def listar_atendimentos(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_agendamento:5002/atendimentos/listar",
            json=payload
        )
...
def editar_atendimento(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_agendamento:5002/{endpoint}/atendimentos/editar",
            json=payload
        )
        print(f"\n>>> Resposta do Servidor ({args.perfil_operador.upper()}):")
        print(resp.json())
