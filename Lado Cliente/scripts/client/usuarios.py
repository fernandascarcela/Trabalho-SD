...
import requests
import sys
from scripts.utils.validacoes import email_valido, cpf_valido, senha_valida, crm_valido
...
def criar_usuario(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_usuarios:5001/{endpoint}/usuarios/registrar",
            json=payload
        )
...
def editar_usuario(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_usuarios:5001/{endpoint}/usuarios/editar",
            json=payload,
            timeout=5
        )
        print(payload)
...
def excluir_usuario(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_usuarios:5001/{endpoint}/usuarios/excluir",
            json=payload
        )
        print(f"\n>>> Resposta do Servidor ({args.perfil_operador.upper()}):")
        print(resp.json())
...
def listar_usuarios(args):
    ...
    try:
        resp = requests.post(
            f"http://interface_usuarios:5001/admin/usuarios/listar",
            json=payload
        )
        print(f"\n>>> Resposta do Servidor ({args.perfil_operador.upper()}):")
        print(resp.json())
